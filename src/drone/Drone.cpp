...
// Documentation is very poor, see this for modes : https://discuss.px4.io/t/mav-cmd-do-set-mode-all-possible-modes/8495
// Each mode have 3 parts : base_mode, mode and submode
// All of those changes without any logic, so hard coded messages are needed

int Drone::setMode_position()
{
...
int Drone::testThrottle(int motorId)
{
    mavlink_message_t message;
    mavlink_command_long_t command;
    command = mavlink_newCommand();
    ...
    command.confirmation = true;
    command.param1 = motorId;  //Hold mode
    command.param2 = MOTOR_TEST_THROTTLE_PERCENT;
    command.param3 = 10;
    command.param4 = 0;
    command.param5 = 0;
    command.param6 = MOTOR_TEST_ORDER_DEFAULT;
